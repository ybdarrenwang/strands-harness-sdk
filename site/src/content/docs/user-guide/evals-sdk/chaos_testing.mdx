...
Chaos testing systematically evaluates agent resilience by injecting controlled failures into tool execution. Using `ChaosPlugin`, `ChaosCase`, and `ChaosExperiment`, you can test how agents handle tool timeouts, network errors, and corrupted responses without modifying agent code.
...
from strands_evals.chaos import ChaosCase, Timeout
...
report = experiment.run_evaluations(task=task_function)
report.run_display()
```
...
from strands_evals.chaos.effects import NetworkError, ExecutionError, ValidationError
...
evaluators = [
    PartialCompletionEvaluator(),         # How much was completed?
    FailureCommunicationEvaluator(),      # Did the agent tell the user?
    RecoveryStrategyEvaluator(),          # Did the agent try alternatives?
]
...
def compare_agents_under_chaos(chaos_cases, configs):
    ...
    for config_name, system_prompt in configs.items():
        ...
        report = experiment.run_evaluations(task=make_task(system_prompt))
...
Start with your most critical tool against all effect types before expanding to multi-tool scenarios. Single-tool tests reveal individual tool robustness; compound tests reveal pipeline fragility:

```python
# Single failure first
